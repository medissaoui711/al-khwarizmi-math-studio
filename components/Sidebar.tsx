...
const Sidebar: React.FC<SidebarProps> = ({ onExampleClick, isOpen, toggleSidebar }) => {
  return (
    <>
      ...
      {/* Sidebar Content */}
      <aside className={`
        fixed top-0 right-0 h-full w-64 bg-white border-l border-slate-200 shadow-xl z-30 transition-transform duration-300 ease-in-out
      ...
      `}>
        <div className="p-6 h-full flex flex-col">
          <div className="flex items-center gap-3 mb-8 text-teal-700">
            ...
            <h1 className="text-xl font-bold font-['Cairo']">الخوارزمي</h1>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="space-y-2">
              {EXAMPLES.map((ex, idx) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 text-slate-700 transition-colors text-right group"
                >
                  <ex.icon className="w-5 h-5 text-slate-400 group-hover:text-teal-600 transition-colors" />
            ...
            <div className="mt-8 p-4 bg-teal-50 rounded-xl border border-teal-100">
              <h3 className="text-teal-800 font-bold mb-2 text-sm">عن التطبيق</h3>
          ...
          <div className="mt-auto pt-6 border-t border-slate-100 text-center">
            <p className="text-xs text-slate-400">© 2024 استوديو الخوارزمي</p>
          </div>
        </div>
      </aside>
    </>
